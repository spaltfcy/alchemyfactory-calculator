...
type SourceMode = 'buy' | 'external' | 'unresolved';
type SourceBucket = Map<string, number>;

type AlphaSolveTrace = {
  mode: 'linear-alpha4' | 'legacy-fallback';
...
export type AlphaLinearSolveResult = {
...
export function calculateAlphaLinear(input: CalculateInput, diagnostics: LinearModelDiagnostics): AlphaLinearSolveResult {
  ...
  for (const demand of analysis.demandLots) {
    ...
    if (cycleRate > EPS) {
      ...
      remaining -= cycleRate;
      pushFlow(makeFlow({ type: 'itemSource', itemId: demand.itemId, sourceMode: 'buy' }, consumer, demand.itemId, cycleRate, demand.role, conveyorItemsPerMinute));
    }
  ...
  return {
    ...
    trace: {
      ...
      notesJa: ['v0.7.0-alpha.4 の新solver試験結果です。内部燃料/肥料など一部はレガシー互換フォールバックします。'],
      notesEn: ['New solver trial result for v0.7.0-alpha.4. Some cases still use the legacy-compatible fallback.'],
    },
  };
}
