...
export type SolverEngineId = 'legacy-v0610' | 'balance-v070-alpha8';
...
const ACTIVE_ENGINE: SolverEngineId = 'balance-v070-alpha8';
const EPS = 1e-9;
...
function compareResults(legacy: CalculationResult, next: CalculationResult): SolverComparisonDiff {
  ...
  const nextSummary = summarizeResult('balance-v070-alpha8', next);
...
export function buildLinearModelDiagnostics(input: CalculateInput): LinearModelDiagnostics {
  ...
  return {
    mode: 'diagnostic-only',
    noteJa:
      'v0.7.0-alpha.8 では、収支ベースsolver結果経路を通常計算に使い、ログ出力時は旧solver比較も併記します。',
    noteEn:
      'v0.7.0-alpha.8 uses the balance-based solver result path at runtime and keeps the legacy comparison in debug/log output.',
    plannedPolicies: {
...
export function buildSolverComparisonFromResults(
  ...
  return {
    ...
    next: summarizeResult('balance-v070-alpha8', newResult),
