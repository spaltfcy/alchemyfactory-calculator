...
export function SettingsTab({ state, setState }: SettingsTabProps) {
  ...
  return (
    <div className="settings-split-v019">
      <section className="panel settings-recipes-v019">
        <h2>{lang === 'ja' ? 'レシピ設定' : 'Recipe settings'}</h2>

        <div className="settings-recipes-list-v019">
          {recipeItems.map((item) => {
            const recipes = getRecipesProducing(item.id);
            return (
              <div className="settings-recipe-row-v019" key={item.id}>
                <div className="settings-label-v019">{text(item.name, lang)}</div>
                <select
      ...
      <div className="settings-main-v019">
        <section className="panel settings-card-v019">
          <h2>{t('language', lang)}</h2>
          <div className="settings-fields-v019">
            <div className="settings-field-v019">
              <div className="settings-label-v019">{t('language', lang)}</div>
        ...
        <section className="panel settings-card-v019">
          ...
          <div className="settings-fields-v019">
            <div className="settings-field-v019">
              <div className="settings-label-v019">{t('machineRounding', lang)}</div>
            ...
            <div className="settings-field-v019">
              <div className="settings-label-v019">{t('defaultSurplusPolicy', lang)}</div>
        ...
        <section className="panel settings-card-v019">
          ...
          <div className="settings-fields-v019">
            <div className="settings-field-v019">
              <div className="settings-label-v019">{lang === 'ja' ? 'グラフ詳細度' : 'Graph detail'}</div>
        ...
        <section className="panel settings-card-v019">
          ...
          <div className="settings-fields-v019">
            ...
            <div className="settings-field-v019">
              <div className="settings-label-v019">{t('importJson', lang)}</div>
        ...
        <section className="panel settings-abilities-v019">
          <h2>{t('abilities', lang)}</h2>
          <div className="ability-grid-v019">
            {ABILITY_IDS.map((id) => (
              <div className="settings-field-v019" key={id}>
                <div className="settings-label-v019">{abilityLabels[id][lang]}</div>
                <input
                  type="number"
                  min="0"
                  step="1"
