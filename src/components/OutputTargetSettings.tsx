...
export type OutputTargetSettingsProps = {
  lang: Lang;
  ...
  makeTarget: () => ProductionTarget;
  normalizeTargetPatch: (target: ProductionTarget, patch: Partial<ProductionTarget>) => ProductionTarget;
  getFocusNodeId?: (target: ProductionTarget) => string | undefined;
...
export function sortItemIdsByDisplayName(itemIds: string[], lang: Lang): string[] {
...
export function OutputTargetSettings({
  lang,
  targets,
  title,
  listAriaLabel,
  ...
  function updateTarget(id: string, patch: Partial<ProductionTarget>) {
    const nextTargets = draftTargets.map((target) => {
      ...
      return normalizeTargetPatch(target, patch);
    });
  ...
  function focusTarget(target: ProductionTarget): void {
    const nodeId = getFocusNodeId?.(target);
  ...
  const sectionClass = sectionClassName ? 'item-output-settings panel ' + sectionClassName : 'item-output-settings panel';
    ...
    <section className={sectionClass}>
      <div className="item-output-settings-header">
        ...
        <div className="item-output-toolbar">
          ...
          <button type="button" onClick={() => commitTargets([...draftTargets, makeTarget()])}>
            {t('addTarget', lang)}
          </button>
        </div>
      </div>

      <div className="item-output-list" aria-label={listAriaLabel}>
        {draftTargets.map((target) => (
          <div
            ...
            onDoubleClick={() => focusTarget(target)}
          >
            <label className="item-output-field item-output-item-field" draggable onDragStart={(event) => onDragStart(event, target.id)} onDoubleClick={(event) => event.stopPropagation()}>
              ...
              <select
                ...
                onChange={(event: ChangeEvent<HTMLSelectElement>) => updateTarget(target.id, { outputItemId: event.target.value })}
              >
            ...
            <div className="item-output-field item-output-mode-field" draggable onDragStart={(event) => onDragStart(event, target.id)} onDoubleClick={(event) => event.stopPropagation()}>
              ...
              <select value={target.mode} onChange={(event: ChangeEvent<HTMLSelectElement>) => updateTarget(target.id, { mode: event.target.value as ProductionTarget['mode'] })}>
                <option value="rate">{t('rateShort', lang)}</option>
                <option value="machines">{t('machinesShort', lang)}</option>
              </select>
            </div>
