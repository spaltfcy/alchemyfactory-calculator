...
function targetRecipeNodeId(target: ProductionTarget): string {
  return 'cauldron:auto:' + target.outputItemId;
}

export function CauldronOutputSettings({ lang, targets, targetDefaults, onChange, onFocusGraphNode }: CauldronOutputSettingsProps) {
  const selectableItems = getSelectableCauldronOutputItems(lang);
  ...
  return (
    <section className="item-output-settings panel cauldron-output-settings">
      ...
      <div className="item-output-list" aria-label={lang === 'ja' ? '錬金釜出力' : 'Cauldron outputs'}>
          ...
          >
            ...
            <div className="item-output-field item-output-mode-field" draggable onDragStart={(event) => onDragStart(event, target.id)} onDoubleClick={(event) => event.stopPropagation()}>
              <div className="item-output-mode-heading">
                ...
                <button type="button" className="item-output-remove danger" aria-label={removeLabel} onClick={() => commitTargets(draftTargets.filter((x) => x.id !== target.id))}>
                  ×
                </button>
              </div>
